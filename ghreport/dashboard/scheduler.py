...
class SyncScheduler:
    def __init__(self, db: Database, settings: Settings):
        ...
        self._MAX_ERRORS = 20
        # Serialize all GitHub API access — prevents concurrent requests
        # that compound rate limiting
    ...
    async def sync_one(self, owner: str, repo: str):
        ...
        async with self._sync_lock:
            cooldown = get_rate_limit_until()
            if cooldown and (cooldown - datetime.now()).total_seconds() > 0:
                logger.warning("Skipping sync %s/%s — rate-limited", owner, repo)
                return
    ...
    async def force_sync_one(self, owner: str, repo: str):
        ...
        async with self._sync_lock:
            cooldown = get_rate_limit_until()
            if cooldown and (cooldown - datetime.now()).total_seconds() > 0:
                logger.warning("Skipping force re-sync %s/%s — rate-limited", owner, repo)
                return
            team = _team_for(self.settings, owner, repo)
            key = f"{owner}/{repo}"
            self._active_syncs.add(key)
    ...
    async def backfill(self, owner: str, repo: str, since: datetime):
        ...
        async with self._sync_lock:
            cooldown = get_rate_limit_until()
            if cooldown and (cooldown - datetime.now()).total_seconds() > 0:
                logger.warning("Skipping backfill %s/%s — rate-limited", owner, repo)
                return
